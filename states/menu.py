...
class Menu(State):
    ...
    def play_cursor_sound(self):
        self.game.play_sound(self.game.cursor_sound)

    def play_confirm_sound(self):
        self.game.play_sound(self.game.confirm_sound)

    def play_back_sound(self):
        self.game.play_sound(self.game.back_sound)

    def play_error_sound(self):
        self.game.play_sound(self.game.error_sound)
    ...
    def check_confirm(self, header_text):
        ...
        while need_confirm:
            ...
            self.draw_confirm_box(self.game.game_canvas, header_text)
            self.update_screen()
            self.reset_key()
        return confirm

    def draw_confirm_box(self, surface, header_text):
        middle_x, middle_y = self.game.GAME_W / 2, self.game.GAME_H / 2
        # draw box
        box = pygame.Surface((100, 60))
        box.fill("gray")
        box_rect = box.get_rect(
            center=(self.game.GAME_W / 2, self.game.GAME_H / 2))
        surface.blit(box, box_rect)
        # draw text
        self.game.draw_text(surface, 10, header_text,
                            "yellow", middle_x, middle_y - 10)
        for choice in self.box_text:
            self.game.draw_text(
                surface, choice[0], choice[1], choice[2], middle_x + choice[3], middle_y + choice[4])
    ...
    def update_screen(self):
        self.game.screen.blit(pygame.transform.scale(self.game.game_canvas, (
            self.game.setting_value["SCREEN_W"], self.game.setting_value["SCREEN_H"])), (0, 0))
        pygame.display.flip()
